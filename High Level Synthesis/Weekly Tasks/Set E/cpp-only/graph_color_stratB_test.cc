...
void graph_color(adj_matrix Adj_G, short int &colors_needed){
	...
	row_array neighbours_with_color_checked;
	...
	for (short int run = 0; run<N; run++){
			...
			for (short int col=0; col<N; col++){
				if (!col){
					...
					neighbours_with_color_checked = 0;
				}
				if (current_row_array[col]) if (color_array[col]) neighbours_with_color_checked[col]=1;
			}
			...
			for (short int i=0; i<N; i++){
				...
				if (neighbours_with_color_checked[i]){
